...
export const HomestayGuestSelectionPopup: React.FC<Props> = ({
  ...
  const [beds, setBeds] = useState<(number | string)[]>(initArray(value.children)); 
      ...
      <View className='selection-section'>
        ...
        <View className='options-grid'>
          {displayOptions.map((opt) => {
            const optVal = opt === '自定义' ? 100 : (opt.endsWith('+') ? parseInt(opt) : parseInt(opt));
    ...
    >
      <View className='homestay-popup-container'>
        ...
        <View className='popup-content'>
          {renderGrid('人数', PEOPLE_OPTIONS, people, setPeople, 'people', '人')}
