    ...
    >
      <View className='guest-popup-container'>
        ...
        <View className='popup-content'>
          ...
          {currentChildren > 0 && (
            <View className='child-age-section'>
              ...
              <Text className='section-desc'>请准确维护儿童年龄，以便我们为您查找最合适的房型及优惠</Text>
              <View className='child-list'>
                {Array.from({ length: currentChildren }).map((_, idx) => {
                   const age = currentChildAges[idx] !== undefined ? currentChildAges[idx] : -1;
                   return (
                      <View key={idx} className='child-item' onClick={() => setEditingChildIndex(idx)}>
                        <Text>儿童 {idx + 1}</Text>
        ...
        <ChildAgeSelectionPopup 
          ...
          currentAge={editingChildIndex !== null ? currentChildAges[editingChildIndex] : undefined}
        />
      </View>
    </Popup>
  );
