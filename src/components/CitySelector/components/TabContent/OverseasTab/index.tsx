...
export const OverseasTab: React.FC<OverseasTabProps> = ({ onSelect }) => {
  const [activeKey, setActiveKey] = useState<string>(overseasCategories[0].key);
  
  const currentCategory = overseasCategories.find(c => c.key === activeKey) || overseasCategories[0];
    ...
    <View className='overseas-tab'>
      ...
      <View className='content-container'>
        <View className='category-content'>
          <Text className='category-title'>{currentCategory.subTitle || currentCategory.title}</Text>
