...
export const RankingList: React.FC<RankingListProps> = ({ ranking, onSelect }) => {
  return (
    <View className='ranking-list-card'>
      ...
      <View className='ranking-items'>
        {ranking.items.slice(0, 5).map((item) => (
          <View key={item.id} className='ranking-item' onClick={() => onSelect(item.name)}>
            <View className='item-image-box'>
              <Image src={item.imageUrl || ''} className='item-image' mode='aspectFill' />
              <View className={`rank-badge rank-${item.rank}`}>{item.rank}</View>
            </View>
