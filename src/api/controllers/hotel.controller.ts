...
export const hotelController = {
  async search(req: Request, res: Response) {
    try {
      const { city, checkInDate, checkOutDate, filters, pageNum = 1, pageSize = 20 } = req.body;
  ...
  async getDetail(req: Request, res: Response) {
    try {
      ...
      if (!hotelId) {
        return res.status(400).json({ code: 400, msg: 'Missing hotelId parameter' });
      }

      const hotelDetail = await hotelService.getHotelDetail(Array.isArray(hotelId) ? hotelId[0] : hotelId);
  ...
  async getRoomTypes(req: Request, res: Response) {
    try {
      ...
      const roomTypes = await hotelService.getRoomTypes(Array.isArray(hotelId) ? hotelId[0] : hotelId);
