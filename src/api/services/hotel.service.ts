...
export const hotelService = {
  async searchHotels(city: string, _checkInDate: string, _checkOutDate: string, filters: any, pageNum: number, pageSize: number) {
    // 使用正则表达式进行模糊匹配，忽略大小写
  ...
  async validatePrice(hotelId: string, roomTypeId: string, _checkInDate: string, _checkOutDate: string) {
    const roomType = await RoomTypeModel.findOne({ _id: roomTypeId, hotelId }).exec();
    if (!roomType) {
